...
a = a.replace("  function screenMagazine(){", `  function screenNews(){
    const list = [...MESSAGES, ...MAGAZINE.map(z=>({type:'me', id:z.id, label:z.label, len:z.year+' · '+z.issue, title_en:z.subtitle, title_ro:z.subtitle_ro}))].sort((x,y)=>x.id.localeCompare(y.id));
    return `
      <h1 class="screen-title">${T('News','Noutăți')}</h1>
